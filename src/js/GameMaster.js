...
const GameMaster = (function () {
  ...
  function createInstance(myInterface) {
    ...
    object.updateShadowStatus = function () {
      ...
      for (let i = 0; i < object.data.pokemon.length; i++) {
        const poke = object.data.pokemon[i];
      ...
      $.each(object.data.pokemon, function (index, poke) {
        ...
        const pokemon = new Pokemon(poke.speciesId, 0, battle);
        let entry = object.getPokemonById(poke.speciesId);
        battle.setNewPokemon(pokemon, 0, false);

        // Remove Return and Frustration from legacy move list
        if (entry.legacyMoves) {
          for (let i = 0; i < entry.legacyMoves.length; i++) {
        ...
        if (object.data.shadowPokemon.indexOf(poke.speciesId) > -1) {
          // Get CP at level 25
          const cp = pokemon.calculateCP(0.667934, 0, 0, 0);
          entry.level25CP = cp;
          ...
          if (entry.legacyMoves) {
            for (let i = 0; i < entry.fastMoves.length; i++) {
              let remove = true;
            ...
            for (let i = 0; i < entry.chargedMoves.length; i++) {
              let remove = true;
    ...
    object.generateDefaultIVs = function () {
      $.each(object.data.pokemon, function (index, poke) {
        const leagues = [1500, 2500];
        const battle = new Battle();

        const pokemon = new Pokemon(poke.speciesId, 0, battle);
        const entry = object.getPokemonById(poke.speciesId);
        battle.setNewPokemon(pokemon, 0, false);
        ...
        for (let i = 0; i < leagues.length; i++) {
          ...
          const cp = pokemon.calculateCP(0.79030001, 15, 15, 15);
          const level35cp = pokemon.calculateCP(0.76156384, 15, 15, 15);
          ...
          if (cp > leagues[i]) {
            ...
            const combinations = pokemon.generateIVCombinations('overall', 1, 4096, null, floor);
            ...
            const { level } = combinations[defaultIndex];
            const { ivs } = combinations[defaultIndex];
            const combination = [level, ivs.atk, ivs.def, ivs.hp];
    ...
    object.loadRankingData = function (caller, category, league, cup) {
      ...
      if (!object.rankings[key]) {
        const file = `${webRoot}data/${cup}/${category}/rankings-${league}.json?v=${siteVersion}`;
    ...
    object.loadGroupData = function (caller, group) {
      ...
      if (!object.groups[key]) {
        const file = `${webRoot}data/groups/${group}.json?v=${siteVersion}`;
    ...
    object.loadTeamData = function (league, cup, callback) {
      const key = `${league}${cup}`;

      if (!object.teamPools[key]) {
        const file = `${webRoot}data/training/teams/${cup}/${league}.json?v=${siteVersion}`;
    ...
    object.modifyPokemonEntry = function (id, type, props) {
      $.each(object.data.pokemon, function (index, poke) {
        if (poke.speciesId === id) {
          switch (type) {
            case 'movepool':
              const movepool = props.moveType === 'fast' ? poke.fastMoves : poke.chargedMoves;
    ...
    object.generateFilteredPokemonList = function (
      ...
      let minStats = 3500; // You must be this tall to ride this ride
      ...
      for (let i = 0; i < object.data.pokemon.length; i++) {
        const pokemon = new Pokemon(object.data.pokemon[i].speciesId, 0, battle);
        ...
        if (stats >= minStats) {
          ...
          // Flag to see if an ID filter should override other filters
          ...
          if (allowed) {
            ...
            if (rankingData && overrides) {
              ...
              for (let n = 0; n < rankingData.length; n++) {
                if (pokemon.speciesId === rankingData[n].speciesId) {
                  // Sort by uses
                  const { fastMoves } = rankingData[n].moves;
                  const { chargedMoves } = rankingData[n].moves;
    ...
    object.generatePokemonListFromSearchString = function (str) {
      ...
      const results = []; // Store an array of qualifying Pokemon ID's
      ...
      const tags = object.data.pokemonTags;
      const regions = object.data.pokemonRegions;
      const battle = new Battle();
      ...
      for (let i = 0; i < queries.length; i++) {
        ...
        for (let n = 0; n < object.data.pokemon.length; n++) {
          const pokemon = new Pokemon(object.data.pokemon[n].speciesId, 0, battle);
          ...
          for (let j = 0; j < params.length; j++) {
            ...
            if (param.charAt(0) === '@' && param.length > 2) {
              ...
              if (param === 'legacy' || param === 'special') {
                for (let k = 0; k < pokemon.fastMovePool.length; k++) {
                  if (
                    pokemon.fastMovePool[k].legacy === true ||
                    pokemon.fastMovePool[k].elite === true
                  ) {
                    valid = true;
                  } else if (param === 'special') {
                    if (
                      pokemon.fastMovePool[k].moveId === 'FRUSTRATION' ||
                      pokemon.fastMovePool[k].moveId === 'RETURN'
                    ) {
                      valid = true;
                    }
                  }
                ...
                for (let k = 0; k < pokemon.chargedMovePool.length; k++) {
                  if (
                    pokemon.chargedMovePool[k].legacy === true ||
                    pokemon.chargedMovePool[k].elite === true
                  ) {
                    valid = true;
                  } else if (param === 'special') {
                    if (
                      pokemon.chargedMovePool[k].moveId === 'FRUSTRATION' ||
                      pokemon.chargedMovePool[k].moveId === 'RETURN'
                    ) {
                      valid = true;
                    }
                  }
              ...
              if (param === 'beam') {
                for (let k = 0; k < pokemon.chargedMovePool.length; k++) {
                  ...
                  if (
                    pokemon.chargedMovePool[k].moveId === 'HYPER_BEAM' ||
                    pokemon.chargedMovePool[k].moveId === 'SOLAR_BEAM'
                  ) {
                    valid = true;
                  }
                }
              ...
              // Move name/type serach
                ...
                for (let k = 0; k < pokemon.fastMovePool.length; k++) {
                  if (
                    pokemon.fastMovePool[k].name.toLocaleLowerCase().startsWith(param) ||
                ...
                for (let k = 0; k < pokemon.chargedMovePool.length; k++) {
                  if (
                    pokemon.chargedMovePool[k].name.toLocaleLowerCase().startsWith(param) ||
    ...
    object.overrideMoveset = function (pokemon, league, cup, overrides) {
      ...
      for (let i = 0; i < overrides.length; i++) {
        if (overrides[i].league === league && overrides[i].cup === cup) {
          ...
          const pokemonList = overrides[i].pokemon;

          for (let n = 0; n < pokemonList.length; n++) {
            if (pokemonList[n].speciesId === pokemon.speciesId) {
              ...
              if (pokemonList[n].chargedMoves) {
                for (let j = 0; j < pokemonList[n].chargedMoves.length; j++) {
  ...
  return {
    getInstance(myInterface) {
      if (!instance) {
        instance = createInstance(myInterface);
      }
      return instance;
    },
  };
