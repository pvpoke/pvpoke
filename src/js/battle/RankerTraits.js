		...
		function rankerObject(){
			...
			this.rank = function(league, scenario){
				...
				for(var i = 0; i < rankCount; i++){
					...
					var speedScore = self.calculateSpeed(pokemon);
					rankObj.speed = speedScore;

					var bulk = self.calculateBulk(pokemon);
					rankObj.bulk = bulk;

					var power = self.calculatePower(pokemon);
					rankObj.power = power;
				...
				// Calculate relative scores, best score is 100
				var scoresets = ["speed", "bulk", "power"];

				for(var n = 0; n < scoresets.length; n++){
					var scoreName = scoresets[n];
					rankings.sort((a,b) => (a[scoreName] > b[scoreName]) ? -1 : ((b[scoreName] > a[scoreName]) ? 1 : 0));

					var bestScore = rankings[0][scoreName];

					for(var j = 0; j < rankings.length; j++){
						rankings[j][scoreName] = Math.round((rankings[j][scoreName] / bestScore) * 1000) / 10;
					}
				}

				// With each relative score calculated, calculate overall score

				for(var n = 0; n < rankings.length; n++){
					var overall = ((rankings[n].bulk * 3) + (rankings[n].speed * 2) + (rankings[n].power * 2)) / 7;
					rankings[n].overall = overall;
				}

				rankings.sort((a,b) => (a.overall > b.overall) ? -1 : ((b.overall > a.overall) ? 1 : 0));

				for(var n = 0; n < rankings.length; n++){
					console.log(rankings[n]);
				}
			...
			// Calculate speed score with time to each Charged Move

			this.calculateSpeed = function(pokemon){
				var fastMove = pokemon.fastMove;
				var chargedMoves = pokemon.chargedMoves;

				var turnsToChargedMove = []; // How many turns it takes to reach 4 cycles of each Charged Move

				for(var i = 0; i < chargedMoves.length; i++){
					var turnsToMove = Math.ceil((chargedMoves[i].energy * 4) / pokemon.fastMove.energyGain) * (pokemon.fastMove.cooldown / 500);
					turnsToChargedMove.push(turnsToMove);
				}

				var avgTurnsToMove = turnsToChargedMove[0];

				// Do a weighted average for the speed to the Pokemon's most efficient Charged Move
				if(chargedMoves.length > 1){
					avgTurnsToMove = ((turnsToChargedMove[0] * chargedMoves[0].dpe) +  (turnsToChargedMove[1] * chargedMoves[1].dpe)) / (chargedMoves[0].dpe + chargedMoves[1].dpe);
				}

				// Higher score must be better, so we need to convert low speed values into high scores

				var speedScore = Math.max(180 - avgTurnsToMove, 1);

				return speedScore;
			}

			// Calculate bulk score for each Pokemon, ability to take damage

			this.calculateBulk = function(pokemon){
				var bulk = pokemon.stats.hp * pokemon.stats.def * pokemon.shadowDefMult;

				return bulk;
			}

			// Calculate power score for each Pokemon, ability to deal damage

			this.calculatePower = function(pokemon){
				var fastMove = pokemon.fastMove;
				var chargedMoves = pokemon.chargedMoves;
				var bestMove = chargedMoves[0];

				if((chargedMoves.length > 1)&&(chargedMoves[1].dpe > chargedMoves[0].dpe)){
					bestMove = chargedMoves[1];
				}

				var fastMoveCount = Math.ceil((bestMove.energy * 3) / fastMove.energyGain);
				var fastMoveDamage = fastMoveCount * fastMove.damage;
				var turns = (fastMoveCount * (fastMove.cooldown / 500)) + 3;
				var dps = (fastMoveDamage + (bestMove.damage * 2)) / turns;
			...
			this.calculateConsistency = function(pokemon){
