...
var RankerMaster = (function () {
  ...
  function createInstance() {
    ...
    function rankerObject() {
      ...
      var workerOptions = { pool: 2 };

      // Set worker mode for async processing (no AJAX write inside worker)
      this.setWorkerMode = function (value) {
        workerMode = value;
      };

      this.setWorkerOptions = function (options) {
        workerOptions = Object.assign(workerOptions, options || {});
      };

      this.setWebRoot = function (wr) {
        webRoot = wr;
      };

      this.setPokemonLists = function (pokeList, targetList) {
      ...
      this.setBattleCP = function (cp) {
        battle.setCP(cp);
      };

      this.setBattleCup = function (cup) {
        if (cup && cup.name == "custom") {
          battle.setCustomCup(cup);
        } else if (cup && cup.name) {
          battle.setCup(cup.name);
        }
      };

      this.writeRankingsToServer = function (cup, category, league, rankings) {
        var json = JSON.stringify(rankings);
        return fetch(webRoot + "data/write.php", {
          method: "POST",
          headers: {
          ...
          .then(function (data) {
            console.log("Ranking write saved:", data);
            return data;
          })
          .catch(function (error) {
            console.log("Ranking write error:", error);
            throw error;
          });
      };
      ...
      this.displayRankingData = function (data, callback, options) {
        ...
        var reportProgress = options.onProgress || function () {};
        var reportError = options.onError || function () {};

        if (workerMode && typeof Worker !== "undefined") {
          var workerURL =
            options.workerURL || "js/battle/rankers/RankerWorker.js";
          ...
          for (var i = 0; i < totalScenarios; i++) {
            (function (scenario) {
              ...
              worker.onmessage = function (event) {
                if (event.data.type == "scenario-complete") {
                  ...
                  self
                    .writeRankingsToServer(
                      result.cup.name || result.cup,
                      result.scenario.slug,
                      result.league,
                      result.rankings,
                    )
                    .catch(function (error) {
                      console.error("Write error in worker response:", error);
                      reportError("Write error:" + error);
                    });

                  if (completedScenarios === totalScenarios && callback) {
                    callback(allResults);
                  }
                }
              };
              ...
              worker.onerror = function (err) {
                ...
                reportError(
                  "Worker error: " +
                    (err.message ||
                      "Unknown error at " + err.filename + ":" + err.lineno),
                );
                worker.terminate();
              };
          ...
          for (
            ...
            var saga = scenarios[currentScenarioIndex];
            var r = self.rank(leagues[currentLeagueIndex], saga);
      ...
      this.rankLoop = function (cp, cup, callback, data, options) {
        options = options || {};
        ...
        for (
          ...
          if (moveSelectMode == "auto") {
            ...
            if (!data) {
              ...
              self.displayRankingData(data, callback, displayOptions);
            }
          }
        }

        if (moveSelectMode == "force") {
          // No synchronous interval-based ranking in force mode - results are handled in displayRankingData
          return;
        }
        ...
        if (cup.name != "custom") {
          ...
          if (!workerMode) {
            fetch(webRoot + "data/write.php", {
              ...
              .then((data) => {
                console.log(data);
                rankings = null; // release reference after successful write
              })
              .catch((error) => {
