    ...
    function RankerObject() {
      ...
      this.rank = function (league, shields) {
        ...
        for (let n = 0; n < iterations; n++) {
          for (let i = 0; i < rankCount; i++) {
            ...
            for (let j = 0; j < matches.length; j++) {
              let sc = matches[j].adjRating * rankings[j].scores[n];

              weights += rankings[j].scores[n];

              // Factor overall score by moveset consistency again
              // If this doesn't happen, Pokemon with Hidden Power will filter to the top because they technically beat a lot of top opponents
        ...
        for (let i = 0; i < rankCount; i++) {
          ...
          const matchupCount = 5;

          // Gather 5 worst matchups for counters
          for (
            let j = rankObj.matches.length - 1;
