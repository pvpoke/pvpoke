...
const BattlerMaster = (function () {
  let instance;

  function createInstance(matchHandler) {
    const object = new InterfaceObject(matchHandler);
    ...
    function InterfaceObject(matchHandler) {
      ...
      let players;
      let phase;
      let activePokemon = []; // The currently active Pokemon
      ...
      const maxCharge = 100;
      const chargeRate = 20;
      const chargeDecayRate = 0.5;
      let phaseInterval;
      const chargeTime = 6000;
      const switchTime = 13000;
      let phaseTimer = 4000;
      let countdown = 0;
      ...
      let properties;
      let battleResult;
      let interfaceLockout = 0; // Prevent clicking the shield or switch options too early
      let listenersInitialized = false; // Prevent event listeners from being added twice
      let priorityAssignment = 1; // Assign priority to a player
      let autotap = false; // Whether or not to automatically register fast moves
      let bufferedSwitch = -1; // If autotap is on, preserve the last input switch and attempt to use until successful
      ...
      this.update = function (response) {
        ...
        if (phase !== response.phase) {
          ...
          switch (response.phase) {
            case 'suspend_charged_attack':
              ...
              const $moveBar = $('.controls .move-bar').eq(response.move).clone();
              ...
              $('.battle-window .animate-message .text').html(
                `Tap to charge up ${activePokemon[0].chargedMoves[response.move].name}!`
              );
              $('.switch-window').removeClass('active');
            ...
            case 'animating':
              $('.animate-message .text').html(
                `${activePokemon[response.actor].speciesName} used ${response.moveName}`
              );

              // If we're transitioning from the Charged Move minigame, submit the damage
              if (phase === 'suspend_charged_attack') {
                const chargeMultiplier = Math.min(0.25 + 0.75 * (charge / maxCharge), 1);
        ...
        for (let i = 0; i < response.pokemon.length; i++) {
          const pokemon = response.pokemon[i];

          const $poke = $('.scene .pokemon-container').eq(i);
          const percent = (pokemon.hp / pokemon.stats.hp) * 100;
          ...
          if (activePokemon.length < i || activePokemon[i] !== pokemon) {
            ...
            $('.team-indicator')
              ...
              .find('.types .type')
              .eq(0)
              .attr('class', `type ${pokemon.types[0]}`);
            $('.team-indicator')
              .eq(i)
              .find('.types .type')
              .eq(1)
              .attr('class', `type ${pokemon.types[1]}`);
            ...
            if (i === 0) {
              ...
              for (let n = 0; n < pokemon.chargedMoves.length; n++) {
                const chargedMove = pokemon.chargedMoves[n];
                const $bar = $('.battle-window .move-bar').eq(n);
          ...
          if (i === 0) {
            for (let n = 0; n < pokemon.chargedMoves.length; n++) {
              const chargedMove = pokemon.chargedMoves[n];
              const $bar = $('.battle-window .move-bar').eq(n);
              const chargePercent = Math.min(pokemon.energy / chargedMove.energy, 1);

              $bar.find('.bar').each((index) => {
                let extraEnergy = Math.min(
                  (pokemon.energy - chargedMove.energy * index) / chargedMove.energy,
                  1
                );
                ...
                const position = Math.min(66 - extraEnergy * positionBase, 66);
        ...
        for (let i = 0; i < response.players.length; i++) {
          ...
          for (let j = 0; j < activePokemonCount; j++) {
            if (i === 0) {
        ...
        for (let i = 0; i < response.messages.length; i++) {
          ...
          const $messageItem = $(`<div turn="${response.turn}">${message.str}</div>`);
          ...
          if (message.index === 1 && message.str === 'Blocked!') {
            $('.battle-window .pokemon-container.opponent .shield-sprite-container').addClass(
              'active'
            );
          }
        }
        ...
        $('.battle-window .messages div').each(() => {
          const messageTurn = parseInt($(this).attr('turn'));
        ...
        for (let i = 0; i < response.animations.length; i++) {
          const animation = response.animations[i];
          let fastAnimationOccurred = false;
          let damageAnimationOccurred = false;
          ...
          switch (animation.type) {
            case 'fast':
              ...
              $('.battle-window .scene .pokemon-container')
                ...
                .attr('class', `fast-move-circle ${activePokemon[animation.actor].fastMove.type}`);
              $('.battle-window .scene .pokemon-container')
                .eq(animation.actor)
          ...
          if (damageAnimationOccurred) {
            setTimeout(() => {
              $('.battle-window .scene .pokemon-container .hp.bar-back').removeClass(
                'effective super-effective not-very-effective'
              );
            }, 250);
          }
        }
      ...
      self.completeSwitchAnimation = function () {
        $('.battle-window .pokemon-container.animate-switch').each((index) => {
          const timePassed = time - parseInt($(this).attr('animation-time'));
      ...
      self.displayEndGameStats = function () {
        ...
        for (let k = 0; k < players.length; k++) {
          const team = players[k].getTeam();

          for (let i = 0; i < team.length; i++) {
            const pokemon = team[i];
            const width = (pokemon.battleStats.damage / maxScore) * 100;
            ...
            $('.battle-stats .damage-section')
              ...
              .find('.shield-bar')
              .css('width', '0');
            for (let n = 0; n < pokemon.battleStats.shieldsBurned; n++) {
              $('.battle-stats .damage-section')
                .eq(k)
                .find('.pokemon-entry')
                .eq(i)
        ...
        const team = players[0].getTeam();
        let damageFromShields = 0;
        let shieldsFromShields = 0;
        const shieldsUsed = 2 - players[0].getShields();
        ...
        let opponentName = players[1].getAI().difficultyToString();
      ...
      this.reportBattleAnalytics = function () {
        const battleSummaryStr = `${battle.getCup().name} ${battle.getCP()} difficulty ${
        ...
        for (let i = 0; i < players.length; i++) {
          ...
          for (let n = 0; n < team.length; n++) {
            const pokemon = team[n];
            let pokeStr = `${pokemon.speciesName} ${pokemon.fastMove.abbreviation}`;
            const chargedMoveAbbrevations = [];
            ...
            for (let k = 0; k < chargedMoveAbbrevations.length; k++) {
              if (k === 0) {
            ...
            pokemon.battleStats.score = Math.round(
              pokemon.battleStats.damage + shieldValue * pokemon.battleStats.shieldsBurned
            );
          ...
          for (let n = 0; n < backupPokeStrs.length; n++) {
        ...
        for (let i = 0; i < players.length; i++) {
          ...
          const opponentScore = teamScores[opponentIndex];
          ...
          const battleRating = Math.floor(
            500 * ((maxScore - opponentScore) / maxScore) + 500 * (score / maxScore)
          );
          ...
          const roster = players[i].getRoster();
          const pokeStrArr = [];

          for (let n = 0; n < roster.length; n++) {
            const pokemon = roster[n];
            let pokeStr = `${pokemon.speciesName} ${pokemon.fastMove.abbreviation}`;
            const chargedMoveAbbrevations = [];
          ...
          const rosterStr = pokeStrArr.join(' ');
      ...
      function battleWindowClick(e) {
        ...
        if (
          ...
          const index = $('.battle-window .controls .move-bar').index($target); // Getting the index of the move
