...
// Load AI archetypes
const file = `${webRoot}data/training/teams/featured/featured-july.json?v=1`;
let featuredTeams = [];
...
$.getJSON(file, function (data) {
  ...
  for (let i = 0; i < featuredTeams.length; i++) {
    $('.featured-team-select').append(
      `
        <option value="${featuredTeams[i].slug}">
          ${featuredTeams[i].name} (${featuredTeams[i].cupName})
        </option>
      `
    );
  }
...
const InterfaceMaster = (function () {
  ...
  function createInstance(matchHandler) {
    ...
    function InterfaceObject(matchHandler) {
      ...
      let mode = 'single'; // Single or tournament
      let teamSelectMethod = 'random'; // Random or manual
      let partySize = 3;
      let battle;
      let playerRoster;
      let currentTeam = [];
      ...
      this.init = function () {
        ...
        for (let i = 0; i < 2; i++) {
          const multiSelector = new PokeMultiSelect($('.poke.multi').eq(i));
      ...
      this.openTeamSelect = function (players, roundRecord) {
        ...
        for (let i = 0; i < players.length; i++) {
          ...
          for (let n = 0; n < roster.length; n++) {
            ...
            const $el = $('.switch-window .pokemon').first().clone();
      ...
      function startBattle() {
        ...
        const val = $('.league-cup-select option:selected').val();
        ...
        const teams = [multiSelectors[0].getPokemonList(), multiSelectors[1].getPokemonList()];
        ...
        const autotapOverride = $('.autotap-toggle').hasClass('on');
      ...
      // Event handler for changing the AI's team selection
      function selectFeaturedTeam() {
        const slug = $('.featured-team-select option:selected').val();

        for (let i = 0; i < featuredTeams.length; i++) {
          const team = featuredTeams[i];
          ...
          if (team.slug === slug) {
            ...
            $(`.league-cup-select option[value="${team.league} ${team.cup}"]`).prop(
              'selected',
              'selected'
            );
            ...
            const teamPokemon = multiSelectors[1].getPokemonList();
            $('.featured-team-preview').html('');

            for (let n = 0; n < teamPokemon.length; n++) {
              const pokemon = teamPokemon[n];
              const $row = $('<div class="preview-poke"></div>');
              const $el = $('.switch-window .pokemon').first().clone();
              $el.find('.name').remove();
              $el.find('.cp').remove();
              ...
              let moveStr = pokemon.fastMove.name;
      ...
      function selectRosterPokemon(e) {
        const $el = $(e.target).closest('.pokemon');
