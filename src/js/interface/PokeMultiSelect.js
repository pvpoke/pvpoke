...
function PokeMultiSelect(element) {
    ...
    while (window.localStorage.key(i) !== null) {
      ...
      if (key !== 'undefined' && key.indexOf('google') === -1) {
        $el
          ...
          .append(`<option value="${key}" type="custom">${key}</option>`);
      }
  ...
  this.openPokeSelect = function (index) {
    ...
    if (index === -1) {
      ...
      $('.modal-content').append(
        `
          <div class="center">
            <div class="save-poke button">Add Pokemon</div>
          </div>
        `
      );
      ...
      $('.modal-content').append(
        `
          <div class="center">
            <div class="save-poke button">Save Changes</div>
          </div>
        `
      );
    }
    ...
    $('.modal .save-poke').on('click', () => {
      // Make sure something's selected
      if (!pokeSelector) {
        return false;
      }

      const pokemon = pokeSelector.getPokemon();
  ...
  this.updateListDisplay = function () {
    ...
    for (let i = 0; i < pokemonList.length; i++) {
      ...
      const $item = $(
        `
          <div class="rank ${pokemon.types[0]}" type-1="${pokemon.types[0]}" type-2="${pokemon.types[1]}">
      ...
      const moveList = [pokemon.fastMove];
  ...
  this.quickFillGroup = function (data) {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      const pokemon = new Pokemon(data[i].speciesId, 1, battle);
  ...
  this.quickFillCSV = function (csv) {
    ...
    for (let i = 0; i < arr.length; i++) {
      ...
      const poke = arr[i].split(',');
      const pokeSettings = poke[0].split('-'); // Get the Pokemon ID and shadow type
      const pokemon = new Pokemon(pokeSettings[0].toLowerCase(), 1, battle);
  ...
  this.convertListToJSON = function () {
    ...
    for (let i = 0; i < pokemonList.length; i++) {
      ...
      for (let n = 0; n < pokemonList[i].chargedMoves.length; n++) {
  ...
  this.convertListToCSV = function () {
    ...
    for (let i = 0; i < pokemonList.length; i++) {
      ...
      for (let n = 0; n < 2 - pokemonList[i].chargedMoves.length; n++) {
        csv += ',none';
      }
  ...
  this.saveCustomList = function (name, isNew) {
    const csv = self.convertListToCSV();
  ...
  this.calculateCliffhangerPoints = function () {
    ...
    for (let i = 0; i < gm.data.cliffhangerTiers.length; i++) {
    ...
    for (let i = 0; i < pokemonList.length; i++) {
      const searchId = pokemonList[i].speciesId.replace('_shadow', ''); // Do this so Shadow and non-Shadow ID's match
  ...
  $el.on('click', '.rankings-container .remove', () => {
    ...
    $('.modal .remove-poke-confirm .yes').click(() => {
  ...
  $el.find('.quick-fill-select').change(() => {
  ...
  $el.find('.export-btn').click(() => {
    ...
    // Copy list text
    $('.modal .list-export .copy').click((e) => {
      const el = $(e.target).prev()[0];
      el.focus();
    ...
    // Import list text
    $('.modal .button.import').on('click', () => {
      const data = $('.modal textarea.list-text').val();
  ...
  $('body').on('click', '.modal textarea.list-text', () => {
    this.setSelectionRange(0, this.value.length);
  });

  // Open the save window
  $el.find('.save-btn').click(() => {
    const selectedGroupType = $(`.quick-fill-select option[value='${selectedGroup}']`).attr('type');
  ...
  // Save data to cookie
  $('body').on('click', '.modal .button.save', () => {
  ...
  $el.find('.delete-btn').click(() => {
    const name = $el.find('.quick-fill-select option:selected').html();
    ...
    $('.modal .delete-list-confirm .button.yes').click(() => {
  ...
  // Change IV settings
  $el.find('.default-iv-select').on('change', () => {
  ...
  $el.find('.format-select').on('change', () => {
