...
function PokeSelect(element, i) {
  const $el = element;
  const $pokeSelect = $el.find('select.poke-select');
  ...
  const self = this;
  let myInterface;
  let isCustom = false; // Whether or not the Pokemon has custom-set level, IVs, or traits
  let context = 'main';
  const searchArr = []; // Array of searchable Pokemon sorted by priority

  let currentHP; // The currently animated HP
  let currentEnergy; // The currently animated energy
  ...
  this.init = function (pokes, b) {
    ...
    $.each(pokemon, function (n, poke) {
      ...
      $pokeSelect.append(
        `<option value="${poke.speciesId}" type-1="${poke.types[0]}" type-2="${poke.types[1]}">${poke.speciesName}</option>`
      );
    });

    searchArr.sort((a, b) => (a.priority > b.priority ? -1 : b.priority > a.priority ? 1 : 0));

    myInterface = InterfaceMaster.getInstance();
  ...
  this.update = function () {
    if (selectedPokemon) {
      ...
      const overall = Math.round(
        (selectedPokemon.stats.hp * selectedPokemon.stats.atk * selectedPokemon.stats.def) / 1000
      );
      const effectiveOverall = Math.round(
      ...
      $el
        .find('.hp .bar')
        .css('width', `${(selectedPokemon.hp / selectedPokemon.stats.hp) * 100}%`);
      ...
      for (let i = 0; i < selectedPokemon.types.length; i++) {
        const typeStr =
          selectedPokemon.types[i].charAt(0).toUpperCase() + selectedPokemon.types[i].slice(1);
        if (selectedPokemon.types[i] !== 'none') {
          $el
            .find('.types')
            .append(`<div class="type ${selectedPokemon.types[i]}">${typeStr}</div>`);
        }
      }

      // Set move selects
      const $fastSelect = $el.find('.move-select.fast');
      ...
      if ($fastSelect.html() === '') {
        ...
        for (let i = 0; i < selectedPokemon.fastMovePool.length; i++) {
          const move = selectedPokemon.fastMovePool[i];
        ...
        $el.find('.move-select.charged').each(() => {
          $(this).append('<option value="none">None</option');

          for (let i = 0; i < selectedPokemon.chargedMovePool.length; i++) {
            const move = selectedPokemon.chargedMovePool[i];
      ...
      for (let i = 0; i < $el.find('.move-select.charged').length; i++) {
        if (i < selectedPokemon.chargedMoves.length) {
          const chargedMove = selectedPokemon.chargedMoves[i];
  ...
  this.animateHealth = function (amount) {
    const health = Math.max(0, selectedPokemon.startHp - amount);
  ...
  this.animateEnergy = function (index, amount) {
    ...
    const energy = selectedPokemon.startEnergy + amount;
    const $bar = $el.find('.move-bar').eq(index);

    $bar.find('.bar').each((i) => {
      const extraEnergy = energy - selectedPokemon.chargedMoves[index].energy * i;
  ...
  this.animateDamage = function (amount) {
    ...
    const position = Math.max(
      Math.ceil($el.find('.hp .bar').eq(0).width() - $el.find('.hp .bar.damage').width()),
      0
    );
  ...
  this.reset = function () {
    ...
    if (myInterface.resetSandbox) {
      myInterface.resetSandbox();
    }
  };
  ...
  $pokeSelect.on('change', (e, fromURL) => {
    const id = $pokeSelect.find('option:selected').val();
    ...
    const value = parseInt($el.find('.shield-select option:selected').val());
    ...
    if (myInterface.resetSelectedPokemon) {
      myInterface.resetSelectedPokemon();
    }
  ...
  $el.find('.move-select').on('change', (e) => {
    const moveId = $(this).find('option:selected').val();
    const moveSlotIndex = $el.find('.move-select.charged').index($(this));
    ...
    if (moveId !== 'custom') {
      ...
      const isFastMove = $(e.target).hasClass('fast');
      ...
      for (let i = 0; i < gm.data.moves.length; i++) {
        ...
        if (
          ((isFastMove && move.energyGain > 0) || (!isFastMove && move.energyGain === 0)) &&
          selectedPokemon.fastMovePool.indexOf(move.moveId) === -1 &&
          selectedPokemon.chargedMovePool.indexOf(move.moveId) === -1
        ) {
          const $option = $(
            `<option type="${move.type}" value="${move.moveId}">${move.name}</option>`
          );
      ...
      $('.modal')
        ...
        .find('.add-move')
        .on('click', () => {
          const moveId = $('.modal').last().find('.move-select option:selected').val();
          const moveType = isFastMove ? 'fast' : 'charged';

          const pool = isFastMove ? selectedPokemon.fastMovePool : selectedPokemon.chargedMovePool;
  ...
  $el.find('.poke-search').on('keyup', () => {
    const searchStr = $el.find('.poke-search').val().toLowerCase();

    if (searchStr === '') return;

    for (let i = 0; i < searchArr.length; i++) {
      const pokeName = searchArr[i].speciesName.toLowerCase();
  ...
  $el.find('.shield-select').on('change', () => {
    const value = parseInt($el.find('.shield-select option:selected').val());
  ...
  // Enter starting HP
  $el.find('.start-hp').on('keyup change', () => {
    const value = parseInt($el.find('.start-hp').val());
  ...
  $el.find('.start-energy').on('keyup change', () => {
    const value = parseInt($el.find('.start-energy').val());
  ...
  $el.find('.check.shield-baiting').on('click', () => {
  ...
  $el.find('.check.priority').on('click', (e) => {
  ...
  $el.find('.check.negate-fast-moves').on('click', (e) => {
    selectedPokemon.negateFastMoves = $(e.target).hasClass('on') ? 0 : 1;
  });

  // Toggle the advanced options drawer
  $el.find('.advanced-section a').on('click', (e) => {
    e.preventDefault();
    $el.find('.advanced-section').toggleClass('active');
  });

  // Turn maximize stats on and off
  $el.find('.maximize-stats').on('click', () => {
    const sortStat = $el.find('.maximize-section .check-group .check.on').first().attr('value');
    const levelCap = parseInt(
      $el.find('.maximize-section .level-cap-group .check.on').first().attr('value')
    );
  ...
  $el.find('.maximize-section .check').on('click', (e) => {
    $(e.target).closest('.check').parent().find('.check').removeClass('on');
  });

  // Select an option from the form section
  $el.find('.form-group .check').on('click', (e) => {
    $(e.target).closest('.check').parent().find('.check').removeClass('on');
  });

  // Change a form option
  $el.find('.form-group .check').on('change', (e) => {
    const formType = $(e.target).attr('value');
  ...
  $el.find('.restore-default').on('click', () => {
  ...
  // Change level input
  $el.find('input.level').on('keyup change', () => {
    const value = parseFloat($el.find('input.level').val());
  ...
  // Change level input
  $el.find('input.iv').on('keyup change', () => {
  ...
  $el.find('input.stat-mod').on('keyup change', () => {
    ...
    if (value >= -4 && value <= 4 && value % 1 === 0) {
      // Valid level
      let attackValue = parseInt($el.find("input.stat-mod[iv='atk']").val());
      let defenseValue = parseInt($el.find("input.stat-mod[iv='def']").val());
  ...
  $el.on('mousemove', '.move-bar', (e) => {
    $tooltip.show();

    $tooltip.attr('class', 'tooltip');

    const index = $el.find('.move-bar').index($(e.target).closest('.move-bar'));
    const move = selectedPokemon.chargedMoves[index];
    ...
    if (battle.getOpponent(selectedPokemon.index)) {
      const opponent = battle.getOpponent(selectedPokemon.index);
      const effectiveness = opponent.typeEffectiveness[move.type];
      displayDamage = battle.calculateDamageByStats(
        selectedPokemon.stats.atk * opponent.shadowAtkMult,
        opponent.stats.def * selectedPokemon.shadowDefMult,
        effectiveness,
        move
      );
    }

    const dpe = Math.floor((displayDamage / move.energy) * 100) / 100;
    const percent = Math.floor((displayDamage / opponent.hp) * 1000) / 10;

    $tooltip.find('.name').html(move.name);
    $tooltip.addClass(move.type);
    $tooltip.find('.details').html(
      `
        ${displayDamage} (${percent}%) <span class="label">dmg</span>
        <br />
        ${move.energy} <span class="label">energy</span>
        <br />
        ${dpe} <span class="label">dpe</span>
      `
    );
  ...
  $el.find('.clear-selection').on('click', (e) => {
    e.preventDefault();
  ...
  $el.find('.pull-from-timeline').on('click', () => {
  ...
  $el.find('.add-fast-move').on('click', () => {
    const startEnergy = Math.min(
      selectedPokemon.startEnergy + selectedPokemon.fastMove.energyGain,
      100
    );
  ...
  // Randomize selection
  $el.find('.random').on('click', (e) => {
    e.preventDefault();

    self.clear();

    const list = $pokeSelect.find('option').not('.hide');
