...
function InterfaceObject() {
  ...
  const rankingInterface = InterfaceMaster.getInstance();
  const pokeSelectors = [];
  let customMetaSelector;
  let overrideSelector;
  let rankingsRunning = false;
  let loadingDataFromCup = false; // Flag for whether we want to import new data into the moveset overrides
  const self = this;
  ...
  // Filter lists
  let filterLists = [cup.include, cup.exclude]; // Array for easy reference

  this.init = function () {
    const { data } = GameMaster.getInstance();
  ...
  this.updateFilterDisplay = function () {
    $('.filters').each((index) => {
      const $el = $(this);
      const filters = filterLists[index];

      if (filters.length > 0) {
        $el.html('');

        for (let i = 0; i < filters.length; i++) {
          const $filter = $('.filter.clone').clone();
  ...
  this.updateFilterValues = function ($el) {
    const listIndex = $el.closest('.filters').attr('list-index');
    const filter = filterLists[listIndex][parseInt($el.attr('index'))];
    ...
    switch (filter.filterType) {
      case 'type':
        $el.find('.type .check.on').each(() => {
          filter.values.push($(this).attr('value'));
        });
        break;

      case 'tag':
        $el.find('.tag .check.on').each(() => {
          filter.values.push($(this).attr('value'));
        });
        break;

      case 'id':
        const str = $el.find('input.ids').val().toLowerCase().replace(/ /g, '');
  ...
  this.receiveRankingData = function (data) {
    if (ranker.getMoveSelectMode() === 'auto') {
      ...
      const overrides = [];
      const group = overrideSelector.getPokemonList();
  ...
  this.importCupSettings = function (data) {
    ...
    for (let i = 0; i < cup.exclude.length; i++) {
  ...
  function addFilter(e) {
    const listIndex = parseInt($(e.target).attr('list-index'));
    const filters = filterLists[listIndex];
  ...
  function changeScenarioShields() {
    const index = parseInt($(this).attr('index'));
    const val = parseInt($(this).find('option:selected').val());
    const allowed = [0, 1, 2];
  ...
  function selectCup() {
    const val = $('.cup-select option:selected').val();
    const c = gm.getCupById(val);
  ...
  // Copy list text
  $('.custom-rankings-import .copy').click((e) => {
  ...
  // Copy text to import
  $('.custom-rankings-import textarea.import').on('click', () => {
    this.setSelectionRange(0, this.value.length);
  });

  // Import text
  $('.custom-rankings-import textarea.import').on('change', () => {
