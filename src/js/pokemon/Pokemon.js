...
function Pokemon(id, i, b) {
      ...
      switch (defaultMode) {
        ...
        case 'gamemaster':
          if (maxCP === 10000) {
            ...
            const combination = data.defaultIVs[`cp${maxCP}`];
    ...
    while (hpIV >= floor) {
      ...
      while (defIV >= floor) {
        ...
        while (atkIV >= floor) {
          ...
          if (calcCP <= targetCP) {
            const atk = cpm * (self.baseStats.atk + atkIV) * self.shadowAtkMult;
            const def = cpm * (self.baseStats.def + defIV) * self.shadowDefMult;
            const hp = Math.floor(cpm * (self.baseStats.hp + hpIV));
            overall = hp * atk * def;
  ...
  this.calculateBreakpoints = function (defender) {
    const effectiveness = defender.typeEffectiveness[self.fastMove.type];
    const minAttack = self.generateIVCombinations('atk', -1, 1)[0].atk;
    const maxAttack = self.generateIVCombinations('atk', 1, 1)[0].atk;
    const maxDefense = defender.generateIVCombinations('def', 1, 1)[0].def;
    const minDamage = battle.calculateDamageByStats(
      minAttack,
      defender.stats.def * defender.shadowDefMult,
      effectiveness,
      self.fastMove
    );
    const maxDamage = battle.calculateDamageByStats(
      maxAttack,
      defender.stats.def * defender.shadowDefMult,
      effectiveness,
      self.fastMove
    ...
    for (let i = minDamage; i <= maxDamage; i++) {
      const breakpoint = battle.calculateBreakpoint(
        i,
        defender.stats.def * defender.shadowDefMult,
        effectiveness,
        self.fastMove
      );
      let maxDefenseBreakpoint = battle.calculateBreakpoint(
        i,
        maxDefense,
        effectiveness,
        self.fastMove
  ...
  this.calculateBulkpoints = function (attacker) {
    const effectiveness = self.typeEffectiveness[attacker.fastMove.type];
    const minDefense = self.generateIVCombinations('def', -1, 1)[0].def;
    const maxDefense = self.generateIVCombinations('def', 1, 1)[0].def;
    const maxAttack = attacker.generateIVCombinations('atk', 1, 1)[0].atk * attacker.shadowAtkMult;
    const minDamage = battle.calculateDamageByStats(
      attacker.stats.atk * attacker.shadowAtkMult,
      maxDefense,
      effectiveness,
      attacker.fastMove
    );
    ...
    for (let i = minDamage; i <= maxDamage; i++) {
      const bulkpoint = battle.calculateBulkpoint(
        i,
        attacker.stats.atk * attacker.shadowAtkMult,
        effectiveness,
        attacker.fastMove
      );
      let maxAttackBulkpoint = battle.calculateBulkpoint(
        i,
        maxAttack,
        effectiveness,
        attacker.fastMove
  ...
  this.resetMoves = function () {
    ...
    if (this.chargedMoves.length > 0) {
      ...
      if (self.activeChargedMoves.length > 1) {
        ...
        if (
          self.activeChargedMoves[1].energy === self.activeChargedMoves[0].energy &&
          self.activeChargedMoves[1].buffs &&
          !self.activeChargedMoves[1].selfDebuffing
        ) {
        ...
        if (
          self.activeChargedMoves[1].energy - self.activeChargedMoves[0].energy <= 10 &&
          self.activeChargedMoves[0].selfAttackDebuffing &&
          !self.activeChargedMoves[1].selfDebuffing
        ) {
          const move = self.activeChargedMoves[0];
          self.activeChargedMoves.splice(0, 1);
      ...
      for (let i = 0; i < self.activeChargedMoves.length; i++) {
        ...
        if (
          (move.dpe - self.bestChargedMove.dpe > 0.03 && move.moveId !== 'SUPER_POWER') ||
          move.dpe - self.bestChargedMove.dpe > 0.3
        ) {
        ...
        if (
          Math.abs(move.dpe - self.bestChargedMove.dpe) < 0.03 &&
          self.bestChargedMove.buffs &&
          move.buffs &&
          move.buffApplyChance > self.bestChargedMove.buffApplyChance &&
          !move.selfDebuffing
        ) {
          self.bestChargedMove = self.activeChargedMoves[i];
        }
    ...
    if (move.energy > 0) {
      ...
      if (move.buffs) {
        ...
        if (move.buffTarget === 'self' && move.buffs[0] > 0) {
          // Factor in a rough number of times the move will be used in battle
  ...
  this.generateMoveUsage = function (opponent, weightModifier) {
    ...
    for (let i = 0; i < chargedMoves.length; i++) {
      ...
      move.uses =
        (Math.pow(move.damage, 2) / Math.pow(move.energy, 4)) * Math.pow(statChangeFactor, 2);
    }
    ...
    for (let i = 1; i < chargedMoves.length; i++) {
      for (let n = 0; n < i; n++) {
        if (
          chargedMoves[i].type === chargedMoves[n].type &&
          chargedMoves[i].energy >= chargedMoves[n].energy &&
          chargedMoves[i].dpe / chargedMoves[n].dpe < 1.3
        ) {
          chargedMoves[i].uses *= 0.5;
          break;
        }
      }
  ...
  this.calculateCycleDPT = function (fastMove, chargedMove) {
    ...
    const cycleDamage =
      cycleFastMoves * fastMove.damage + chargedMove.damage * (150 / chargedMove.energy - 1) + 1; // Emulate TDO with a shield
  ...
  // JavaScript functions declared like this can't be called until after the declaration
  // I need to call this in the constructor
  // So instead of moving the one line of code beneath the function declaration, I chose ... this.
  ...
  this.setLevel = function (amount, initialize) {
    ...
    if (index % 1 === 0) {
      ...
      // Set CPM for half levels
      self.cpm = Math.sqrt(
        (Math.pow(cpms[Math.floor(index)], 2) + Math.pow(cpms[Math.ceil(index)], 2)) / 2
      );
    }
  ...
  this.applyStatBuffs = function (buffs) {
    const { maxBuffStages } = gm.data.settings;
  ...
  this.getEffectiveStat = function (index) {
    const { buffDivisor } = gm.data.settings;
    let multiplier;
  ...
  this.generateURLMoveStr = function () {
    ...
    const fastMoveIndex = self.fastMovePool.indexOf(self.fastMove);
    const chargedMove1Index = self.chargedMovePool.indexOf(self.chargedMoves[0]) + 1;
    const chargedMove2Index = self.chargedMovePool.indexOf(self.chargedMoves[1]) + 1;
  ...
  this.calculateConsistency = function () {
    ...
    if (chargedMoves.length === 2) {
      const effectivenessScenarios = [[1, 1]];
      ...
      for (let n = 0; n < effectivenessScenarios.length; n++) {
        ...
        chargedMoves[0].dpe =
          (chargedMoves[0].damage / chargedMoves[0].energy) * effectivenessScenarios[n][0];
        chargedMoves[1].dpe =
          (chargedMoves[1].damage / chargedMoves[1].energy) * effectivenessScenarios[n][1];
        chargedMoves.sort((a, b) => (a.dpe > b.dpe ? -1 : b.dpe > a.dpe ? 1 : 0));
        ...
        const cycleFastDamage = fastMove.damage * cycleFastMoves;
        ...
        if (
          ...
          if (chargedMoves[1].energy < chargedMoves[0].energy) {
            factor +=
              (1 - factor) * ((chargedMoves[1].energy - 30) / (chargedMoves[0].energy - 30)) * 0.5;
          }
        }
